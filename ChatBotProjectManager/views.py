...
def get_database_context(user):
    ...
    try:
     ...
     # Project Manager-specific context

        # 1. Fetch current user
        context["user"] = fetch_table_data(
            "users",
            "SELECT * FROM tbl_users WHERE user_id = %s",
            (user['user_id'],)
        )

        # 2. Fetch department of the user
        context["department"] = fetch_table_data(
            "departments",
            "SELECT * FROM tbl_departments WHERE department_id = %s",
            (user['department_id'],)
        )

        # 3. Fetch all projects under this department
        context["projects"] = fetch_table_data(
            "projects",
            "SELECT * FROM tbl_projects WHERE department_id = %s",
            (user['department_id'],)
        )

        # 4. Fetch all tasks under these projects
        context["project_tasks"] = fetch_table_data(
            "tasks",
            """
            SELECT t.*
            FROM tbl_tasks t
        ...
        # 5. Fetch all users in the same department
        ...
        # 6. Fetch all users working on projects in the same department
        context["project_users"] = fetch_table_data(
            "project_users",
            """
            SELECT DISTINCT u.*
            FROM tbl_users u
            JOIN tbl_tasks t ON u.user_id = t.assigned_to
            JOIN tbl_projects p ON t.project_id = p.project_id
            WHERE p.department_id = %s
            """,
            (user['department_id'],)
        ...
        # 7. Fetch all tasks under projects in the same department
        context["project_user_tasks"] = fetch_table_data(
            "project_user_tasks",
            """
            SELECT t.*
            FROM tbl_tasks t
